cbuffer _0 : register(b0)
{
    ...
    uint2 _TextUIDataSize;
    uint _InvalidFontTextureIndex;
    uint _FontTextureTextLength;
}

Texture2D<float4> _Texture : register(t0);
Texture2D<float4> _FontTexture : register(t1);
Texture2D<uint4> _TextUIDataTexture : register(t2);
...
PSOutput PSMain(V2P input)
{
    ...
    textSamplingParams.TextUIDataTexture = _TextUIDataTexture;
    textSamplingParams.Sampler = _Sampler;
    textSamplingParams.FontTextureSize = _FontTextureSize;
    textSamplingParams.TextUIDataSize = _TextUIDataSize;
    textSamplingParams.PixelPosition = uint2(input.pos.xy);
    textSamplingParams.InvalidFontTextureIndex = _InvalidFontTextureIndex;
    textSamplingParams.FontTextureTextLength = _FontTextureTextLength;
