...
namespace ForiverEngine
{
	...
	struct CameraTransform : public Transform
	{
		...
		// float nearClip
		// float farClip
		// bool isPerspective (4バイトで保存する)
		// float fov
		// float aspectRatio

		static constexpr std::size_t SelfSerializedSize = sizeof(float) * 5;

		static std::string Serialize(const CameraTransform& transform)
		{
			std::string buffer;
			buffer.resize(SelfSerializedSize);

			const std::uint32_t isPerspectiveAsInt = transform.isPerspective ? 1 : 0;

			std::memcpy(buffer.data() + sizeof(float) * 0, &transform.nearClip, sizeof(float));
			std::memcpy(buffer.data() + sizeof(float) * 1, &transform.farClip, sizeof(float));
			std::memcpy(buffer.data() + sizeof(float) * 2, &isPerspectiveAsInt, sizeof(std::uint32_t));
			std::memcpy(buffer.data() + sizeof(float) * 3, &transform.fov, sizeof(float));
			std::memcpy(buffer.data() + sizeof(float) * 4, &transform.aspectRatio, sizeof(float));

			return Transform::Serialize(transform) + buffer;
		}

		static CameraTransform Deserialize(std::string_view buffer)
		{
			if (buffer.size() < Transform::SelfSerializedSize + SelfSerializedSize)
			{
			...
			std::uint32_t isPerspectiveAsInt = 0;
			std::string_view cameraTransformBuffer = buffer.substr(Transform::SelfSerializedSize, SelfSerializedSize);
			std::memcpy(&transform.nearClip, cameraTransformBuffer.data() + sizeof(float) * 0, sizeof(float));
			std::memcpy(&transform.farClip, cameraTransformBuffer.data() + sizeof(float) * 1, sizeof(float));
			std::memcpy(&isPerspectiveAsInt, cameraTransformBuffer.data() + sizeof(float) * 2, sizeof(std::uint32_t));
			std::memcpy(&transform.fov, cameraTransformBuffer.data() + sizeof(float) * 3, sizeof(float));
			std::memcpy(&transform.aspectRatio, cameraTransformBuffer.data() + sizeof(float) * 4, sizeof(float));
			transform.isPerspective = (isPerspectiveAsInt != 0);

			std::string_view baseTransformBuffer = buffer.substr(0, Transform::SelfSerializedSize);
			Transform baseTransform = Transform::Deserialize(baseTransformBuffer);
