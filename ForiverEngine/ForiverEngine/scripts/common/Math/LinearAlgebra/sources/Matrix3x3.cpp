...
namespace ForiverEngine
{
	...
	Matrix3x3 Matrix3x3::FromColumnVectors(
		const Vector3& c0,
		const Vector3& c1,
		const Vector3& c2
	) noexcept
	...
	Matrix3x3 Matrix3x3::FromRowVectors(
		const Vector3& r0,
		const Vector3& r1,
		const Vector3& r2
	) noexcept
	...
	Matrix3x3 Matrix3x3::Scale(float sx, float sy, float sz) noexcept
	{
		return Matrix3x3(
			sx, 0.0f, 0.0f,
			0.0f, sy, 0.0f,
	...
	Matrix3x3 Matrix3x3::Scale(const Vector3& scale) noexcept
	{
		return Scale(scale.x, scale.y, scale.z);
	}
	Matrix3x3 Matrix3x3::Rotate(Quaternion quat) noexcept
	{
	...
	Vector3 Matrix3x3::operator*(const Vector3& vec) const noexcept
	{
		return Vector3(
	...
	Matrix3x3 operator*(float scalar, const Matrix3x3& mat) noexcept
	{
		return Matrix3x3(
	...
	Matrix3x3& Matrix3x3::operator*=(float scalar) noexcept
	{
		c0r0 *= scalar; c1r0 *= scalar; c2r0 *= scalar;
		c0r1 *= scalar; c1r1 *= scalar; c2r1 *= scalar;
		c0r2 *= scalar; c1r2 *= scalar; c2r2 *= scalar;
