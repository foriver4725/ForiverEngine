...
{
	Matrix4x4::Matrix4x4(const Matrix3x3& other) noexcept
	...
	Matrix4x4::Matrix4x4::Matrix4x4(Matrix3x3&& other) noexcept
		: c0r0(other.c0r0), c1r0(other.c1r0), c2r0(other.c2r0), c3r0(0.0f)
		, c0r1(other.c0r1), c1r1(other.c1r1), c2r1(other.c2r1), c3r1(0.0f)
		, c0r2(other.c0r2), c1r2(other.c1r2), c2r2(other.c2r2), c3r2(0.0f)
		, c0r3(0.0f), c1r3(0.0f), c2r3(0.0f), c3r3(1.0f)
	...
	Matrix4x4 Matrix4x4::FromColumnVectors(
		const Vector4& c0,
		const Vector4& c1,
		const Vector4& c2,
		const Vector4& c3
	...
	Matrix4x4 Matrix4x4::FromRowVectors(
		const Vector4& r0,
		const Vector4& r1,
		const Vector4& r2,
		const Vector4& r3
	...
	Matrix4x4 Matrix4x4::Scale(float sx, float sy, float sz) noexcept
	{
		const Matrix3x3 scale3x3 = Matrix3x3::Scale(sx, sy, sz);
		return Matrix4x4(scale3x3);
	};
	Matrix4x4 Matrix4x4::Scale(const Vector3& scale) noexcept
	{
		return Scale(scale.x, scale.y, scale.z);
	}
	Matrix4x4 Matrix4x4::Rotate(Quaternion quat) noexcept
	{
		const Matrix3x3 rot3x3 = Matrix3x3::Rotate(quat);
		return Matrix4x4(rot3x3);
	}
	Matrix4x4 Matrix4x4::Translate(float tx, float ty, float tz) noexcept
	{
		return Matrix4x4(
			1.0f, 0.0f, 0.0f, tx,
			0.0f, 1.0f, 0.0f, ty,
	...
	Matrix4x4 Matrix4x4::Translate(const Vector3& translation) noexcept
	{
	...
	Matrix4x4 operator*(float scalar, const Matrix4x4& mat) noexcept
	{
		return Matrix4x4(
