...
namespace ForiverEngine
{
	...
	public:
		...
		)
		{
			...
			// t1 (フォントテクスチャ)
			const Texture sr1Texture = D3D12Utils::LoadTexture({ "assets/font.png" });
			const GraphicsBuffer sr1 = D3D12Utils::InitSR(device, commandList, commandQueue, commandAllocator, sr1Texture);

			// t2 (データをテクスチャとしてアップロードする)
			const Texture sr2Texture = data.CreateTexture();
			const GraphicsBuffer sr2 = D3D12Utils::InitSR(device, commandList, commandQueue, commandAllocator, sr2Texture);

			// b0
			const CBData0 cbData0 =
			{
				.FontTextureSize = { static_cast<std::uint32_t>(sr1Texture.width), static_cast<std::uint32_t>(sr1Texture.height) },
				.TextUIDataSize = { static_cast<std::uint32_t>(sr2Texture.width), static_cast<std::uint32_t>(sr2Texture.height) },
			...
			Base::Init(device, windowSize, { cb0 }, { { sr1, sr1Texture }, { sr2, sr2Texture } }, "./shaders/Text.hlsl");
		}
