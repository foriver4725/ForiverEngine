...
- 左上にデバッグ情報を表示
- ワールドデータをシリアライズしてセーブ・ロード。複数セーブデータの管理が可能
...
- セーブデータは `saves/world` フォルダに保存される。ファイル名は `<ワールド名>.world`
- ゲーム起動時、`WorldName.txt` に書かれた名前のワールドでプレイする。<br/>セーブデータが無ければ新規作成、あればコンティニュー

| 操作 | 方法 |
| --- | --- |
| ワールドのセーブ | ゲーム終了時にオートセーブされる。<br/>`saves/world/<ワールド名>.world` として保存 |
| ワールドのロード | ゲーム起動時に `WorldName.txt` に書かれた名前を取得し、<br/>`saves/world/<ワールド名>.world` があればそれをロード、無ければ新規作成する。<br/>`WorldName.txt` を編集してワールド名を書き換えることで、<br/>ロードするワールドを切り替えることができる |
| ワールド名を変更 | `saves/world/<ワールド名>.world` の `<ワールド名>` を変更する |
| ワールドを削除 | `saves/world/<削除したいワールド名>.world` を削除する |
...
| マウスホイール・数字キー | アイテムバーの選択を切り替え |
| マウスホイールクリック | 見ているブロックをアイテムバーで選択する |
| エスケープ | ゲーム終了 |
