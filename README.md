...
- ゲーム起動時、`WorldName.txt` に書かれた名前のワールドでプレイする。セーブデータが無ければ新規作成、あればコンティニュー

| 操作 | 方法 |
| --- | --- |
| ワールドのセーブ | ゲーム終了時にオートセーブされる。<br/>`saves/world/<ワールド名>.world` として保存 |
| ワールドのロード | ゲーム起動時に `WorldName.txt` に書かれた名前を取得し、<br/>`saves/world/<ワールド名>.world` があればそれをロード、無ければ新規作成する<br/>`WorldName.txt` を編集してワールド名を書き換えることで、ロードするワールドを切り替えることができる |
